 ...
 * @LastEditTime: 2020-10-01 17:55:57
...
<template>
  <el-card class="box-card">
    <el-page-header content="更新机构信息" style="margin-left: 2%;margin-bottom: 2%;" @back="goBack" />
...
<script>
import { updateOrg } from '@/api/organization'
...
export default {
  ...
  created() {
    this.ruleForm.orgName = this.$route.params.orgName
    this.ruleForm.orgId = this.$route.params.orgId
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true
          updateOrg(JSON.stringify(this.ruleForm)).then(() => {
            this.$message({
              message: '更新成功',
              type: 'success'
            })
